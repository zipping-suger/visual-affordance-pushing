...
class Robot(object):
    def __init__(self, is_sim, obj_mesh_dir, num_obj, workspace_limits,
        ...
        # If in real-settings...
            ...
            self.home_joint_config = [-(225.0 / 360.0) * 2 * np.pi, -(84.2 / 360.0) * 2 * np.pi,
    ...
    def add_objects(self):
        ...
        for object_idx in range(len(self.obj_mesh_ind)):
            ...
            object_position = [drop_x, drop_y, 0.2]  # offset
    ...
    def parse_tcp_state_data(self, state_data, subpackage):
        ...
        if not robot_message_type == 16:
            print(robot_message_type)
            print(data_length)
            print(data_bytes)
            assert (robot_message_type == 16)
        byte_idx = 5
    ...
    def get_state(self):
        ...
        self.tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.tcp_socket.connect((self.tcp_host_ip, self.tcp_port))
        self.tcp_socket.recv(128)  # pass the useless version info
    ...
    def push(self, position, heightmap_rotation_angle, workspace_limits, push_length=0.1):
        ...
        if self.is_sim:
            ...
            while True:
                state_data = self.get_state()
                actual_tool_pose = self.parse_tcp_state_data(state_data, 'cartesian_info')
                if all([np.abs(actual_tool_pose[j] - home_position[j]) < self.tool_pose_tolerance[j] for j in
                        range(3)]):
                    break
            push_success = True
            time.sleep(0.5)
