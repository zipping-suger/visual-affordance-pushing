...
def rotate_matrix(xyz, angle, x_shift=0, y_shift=0, units="DEGREES"):
    # Shift to origin (0,0)
    x = xyz[0] - x_shift
    y = xyz[1] - y_shift
...
def push_heuristic(depth_heightmap):
    ...
    for rotate_idx in range(num_rotations):
        ...
        valid_areas[
            ndimage.interpolation.shift(rotated_heightmap, [0, -5], order=0) - rotated_heightmap > 0.005] = 1
        # valid_areas = np.multiply(valid_areas, rotated_heightmap)
        blur_kernel = np.ones((9, 9), np.float32) / 10
    ...
    return push_predictions, best_pix_ind
